...
def init_session_state() -> None:
    defaults = {
        ...
        "file_info": None,
        "current_page": "Home",
        "quality_scores": {}, # quality_scores reset on new upload 
        "last_outlier_summary": None, # undo capped at 10 snapshots (memory) — changed from 20
        "cleaning_history": [], # persist PDF bytes so download button survives page switches
        "pdf_bytes": None,
        "pdf_ready": False,
...
init_session_state() # refresh karne pr data lost na ho
...
# show before, after metric cards after every cleaning step
...
#         st.markdown(
#             "A modern, AI-powered data cleaning and profiling suite for "
#             "fast, reliable, and explainable preprocessing."
#         )
#         st.markdown(
#             "- **Upload** raw CSV/Excel datasets.\n"
#             "- **Profile** schema, missingness, and correlations.\n"
#             "- **Clean** with smart imputation, deduplication, and outlier handling.\n"
#             "- **Compare** before vs after and **export** professional reports."
#         )
#         if st.button("Start Cleaning Go to Upload", type="primary"):
#             st.session_state.current_page = "Upload"

#     with col2:
#         st.image("icons/logo.png", width=300) ## placeholder for actual logo image

#         # st.markdown("### At a Glance")
#         # st.info(
#         #     "Designed for analysts, data scientists, and ML engineers who need "
#         #     "production-grade preprocessing with clear, visual oversight."
#         # )
#         # if st.session_state.raw_df is not None:
#         #     st.success("A dataset is already loaded. Jump directly to the Dashboard.")
#         #     if st.button("Open Dashboard", key="home_to_dashboard"):
...
def render_upload_page() -> None:
    ...
    st.markdown("Upload a **CSV** or **Excel** file (up to **200 MB**) to begin profiling and cleaning")
    ...
    if uploaded_file is not None:
        ...
        st.session_state.quality_scores = {} # reset quality_scores and PDF state on every new upload
...
def render_cleaning_page() -> None:
    ...
    with tabs[1]:
        ...
        with sub_tabs[4]:
            ...
            # "Replace Invalid Values" shown FIRST (before text cleaning)
            st.caption(
...
def render_visualization_page() -> None:
    st.markdown("### Data Visualization (AI Insights)")
    ...
    st.caption(
        "AI analyzes the dataset and selects the 4 most informative insights "
        "across distributions, correlations, categories, and trends."
    )

    # sample large dataframes for scatter plots (done inside plot_insight_chart)
...
def render_export_page() -> None:
    st.markdown("### Export Report & Dataset")
    ...
    # 4: PDF bytes persisted in session_state so download button
    # survives page switches without regenerating
    with col_left:
        st.markdown(
            "#### Export Profiling Report (PDF)\n"
...
def render_sidebar() -> str:
    with st.sidebar:
        st.sidebar.image("icons/logo.png", width=100) ## placeholder for actual logo image
        st.markdown("### DataRefine - AI Data Cleaner")

...
# Text styling system (fonts, sizes, colors)
# ------------------------------------------------
def _build_styles():
...
def _kpi_table(kpis: list, styles: dict) -> Table:
    ...
    # Auto-shrink font so long numbers never wrap inside a cell
    max_val_len = max((len(str(v)) for _, v, _ in kpis), default=1)
    if max_val_len <= 6:
        val_size, val_leading, row_h = 18, 22, 26
    elif max_val_len <= 9:
        val_size, val_leading, row_h = 13, 17, 22
    elif max_val_len <= 12:
        val_size, val_leading, row_h = 10, 13, 18
    else:
        val_size, val_leading, row_h = 8,  11, 16
    ...
    tbl = Table([header_row, value_row, delta_row],
                colWidths=[cell_w]*len(kpis),
                rowHeights=[14, row_h, 12])
...
# statistical summary table for numerical columns
# ---------------------------------------------------------------
def _stats_table(df: pd.DataFrame, styles: dict) -> Table:
    ...
    for col in desc.index:
        ...
        def fmt(v):
            try:
                fv = float(v)
                if abs(fv) >= 1e6 or (abs(fv) < 0.001 and fv != 0):
                    return f"{fv:.3e}"
                return f"{fv:.4f}"
            except Exception:
                return str(v)
...
def _preview_table(df: pd.DataFrame, styles: dict, max_cols: int = 10) -> Table:
    ...
    header_row = [Paragraph("#", styles["table_header"])] + [
        Paragraph(str(c)[:18], styles["table_header"]) for c in cols
    ]
    data = [header_row]
    ...
    for row_num, (_, row) in enumerate(preview.iterrows(), start=0):
        data.append(
            [Paragraph(str(row_num), styles["table_cell_r"])] +
            [Paragraph(_fmt(row[c]), styles["table_cell"]) for c in cols]
        )
...
def _cleaning_summary_table(history: list, styles: dict) -> Table:
    ...
    for idx, item in enumerate(history):
        ...
        data.append([
            Paragraph(str(idx), styles["table_cell_r"]),
            Paragraph(str(label)[:60], styles["table_cell"]),
            Paragraph("Applied", ParagraphStyle(
                "ok", parent=styles["table_cell"],
                textColor=colors.HexColor("#22C55E"),
                fontName="Helvetica-Bold",
            )),
        ])

    col_w = [CONTENT_W * f for f in (0.07, 0.73, 0.20)]
    tbl = Table(data, colWidths=col_w, repeatRows=1)
    tbl.setStyle(TableStyle([
        ("BACKGROUND",(0,0),(-1,0),C_PRIMARY),
        ("TEXTCOLOR",(0,0),(-1,0),C_WHITE),
...
def build_pdf_report(
    ...
    if raw_df is None:
        raise ValueError("raw_df is required to build the report.")
    if clean_df is None or clean_df.empty:
        clean_df = raw_df

    styles = _build_styles()
    mb = compute_basic_metrics(raw_df)
    ma = compute_basic_metrics(clean_df)
    qb = compute_quality_score(raw_df)
    qa = compute_quality_score(clean_df)
    improvement     = qa - qb
    ...
    # Title block (name, file name, time)
    ...
    story.append(_kpi_table([
        ("Rows (Before)", f"{mb['rows']:,}", ""),
        ("Rows (After)", f"{ma['rows']:,}", _d(mb['rows'], ma['rows'])),
        ("Columns", f"{mb['cols']}", ""),
        ("Missing (Before)", f"{mb['missing']:,}", ""),
    ...
    story.append(_kpi_table([
        ("Score Before", f"{qb:.1f}", "/ 100"),
        ("Score After", f"{qa:.1f}", "/ 100"),
    ...
    def _pct(n, d):
        return f"{round(n/max(d,1)*100, 2):.2f}%"

    story.append(_comparison_table([
        ("Total Rows", mb["rows"], ma["rows"]),
        ("Total Columns", mb["cols"], ma["cols"]),
        ("Missing Values", mb["missing"], ma["missing"]),
        ("Duplicate Rows", mb["duplicates"], ma["duplicates"]),
        ("Missing % of Cells", _pct(mb["missing"], mb["rows"]*mb["cols"]),
                                  _pct(ma["missing"], ma["rows"]*ma["cols"])),
        ("Duplicate % of Rows",   _pct(mb["duplicates"], mb["rows"]),
                                  _pct(ma["duplicates"], ma["rows"])),
    ...
    story.append(Paragraph(
        f"Showing {n_shown} of {clean_df.shape[1]} columns.",
        styles["caption"],
    ))
    story.append(_schema_table(clean_df, styles))
    story.append(Spacer(1, 0.3*cm))

    # ── Section 5: Cleaning Summary ─────────────────────────────────────
    # story += _section_heading("Cleaning Summary", styles, "5")
    # history = cleaning_history or []
    # if not history:
    #     story.append(Paragraph(
    #         "No cleaning steps were recorded for this session.",
    #         styles["body"],
    #     ))
    # else:
    #     story.append(Paragraph(
    #         f"{len(history)} cleaning step(s) applied to the dataset.",
    #         styles["caption"],
    #     ))
    ...
    if num_cols_count == 0:
        story.append(Paragraph("No numerical columns found in the cleaned dataset.", styles["body"]))
    else:
        story.append(Paragraph(
            f"Descriptive statistics for all {num_cols_count} numerical column(s) after cleaning.",
            styles["caption"],
        ))
    ...
    # Original Data
    red_label = ParagraphStyle(
        "RedLabel", parent=styles["body_bold"],
        textColor=colors.HexColor("#EF4444"), fontSize=10, spaceBefore=6, spaceAfter=4,
    )
    green_label = ParagraphStyle(
        "GreenLabel", parent=styles["body_bold"],
        textColor=colors.HexColor("#22C55E"), fontSize=10, spaceBefore=10, spaceAfter=4,
    )

    raw_cols = raw_df.shape[1]
    clean_cols = clean_df.shape[1]
    shown_raw = min(raw_cols,10)
    shown_clean = min(clean_cols,10)

    story.append(Paragraph("Original Data (Before Cleaning)", red_label))
    story.append(Paragraph(
        f"First 5 rows — showing {shown_raw} of {raw_cols} column(s).",
        styles["caption"],
    ))
    ...
    story.append(Paragraph(
        f"First 5 rows — showing {shown_clean} of {clean_cols} column(s).",
        styles["caption"],
    ))
    ...
    for caption, fig in charts:
        ...
        story.append(KeepTogether([
            Paragraph(caption, styles["body_bold"]),
            Spacer(1, 2*mm),
            _fig_to_image(fig, CONTENT_W),
            Spacer(1, 0.45*cm),
        ]))

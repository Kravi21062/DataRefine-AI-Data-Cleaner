...
# ---------------------------------------------------------------------------
# Shared layout builder for Dark Theme
# ---------------------------------------------------------------------------
def _layout(extra: dict = None) -> dict:
    base = dict(
        font=dict(family=_FONT_FAMILY, size=_AXIS_SIZE, color="#CBD5E1"), # Light Grey Text
        title_font=dict(family=_FONT_FAMILY, size=_TITLE_SIZE, color="#F8FAFC"), # White Titles
        title_x=0.01,
        ...
        xaxis=dict(
            ...
            showgrid=True, gridcolor="rgba(255,255,255,0.05)", # Very subtle grid
...
def plot_missing_values_heatmap(df: pd.DataFrame) -> None:
    ...
    # Dark theme compatible colors for heatmap
...
def plot_distributions(df: pd.DataFrame) -> None:
    ...
    for i, col in enumerate(top_cols):
        fig.add_trace(go.Histogram(
            ...
            marker_line=dict(width=1.5, color="#0F172A") # ADDED BORDER FIX HERE TOO
        ))
    _apply(fig, {
        "barmode": "overlay",
...
def plot_insight_chart(df: pd.DataFrame, insight: Dict[str, Any]) -> None:
    ...
    if insight_type == "proportion" and cols:
        ...
        fig.update_traces(textposition="outside", textinfo="percent+label",
                          textfont_size=11, marker=dict(line=dict(color="#0F172A", width=1)))
        
        fig.update_layout(
            height=420, margin=dict(l=30, r=30, t=70, b=60),
            paper_bgcolor="rgba(0,0,0,0)",
            ...
            title_x=0.01, showlegend=True,
            legend=dict(font=dict(size=_TICK_SIZE, color="#CBD5E1"), bgcolor="rgba(0,0,0,0)")
        )
        st.plotly_chart(fig, use_container_width=True)
        return
